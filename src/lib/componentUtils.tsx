import { Circle, Loader, CheckCircle, ChevronsDown, ChevronRight, ChevronsUp } from "lucide-react";
...
export function getStatusBadge(status: TodoStatus): { icon: JSX.Element; color: string } {
    const statusMap: Record<TodoStatus, { icon: JSX.Element; color: string }> = {
        "todo": { icon: <Circle className="w-4 h-4 text-gray-700" />, color: "bg-gray-200 text-gray-800" },
        "in-progress": { icon: <Loader className="w-4 h-4 text-yellow-500 animate-spin" />, color: "bg-yellow-200 text-yellow-800" },
        "done": { icon: <CheckCircle className="w-4 h-4 text-green-500" />, color: "bg-green-200 text-green-800" },
