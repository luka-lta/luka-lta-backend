...
import {getEventDisplayName, PROPS_TO_HIDE} from "@/lib/events.ts";
import {DateTime} from "luxon";
import {formatDuration, hour12} from "@/lib/dateTimeUtils.ts";
import {cn} from "@/lib/utils.ts";
import {EventTypeIcon} from "@/components/EventIcons.tsx";
...
export function PageviewItem({
    ...
    if (isPageview && nextTimestamp) {
        const nextTime = DateTime.fromSQL(nextTimestamp, { zone: "utc" });
            ...
            <div className="flex flex-col ml-3 flex-1">
                <div className="flex items-center flex-1 py-1">
                    ...
                    <div className="flex-1 min-w-0 mr-4">
                        {isPageview ? (
                            <Link
                                to={`https://${item.hostname}${item.pathname}${item.urlParameters ? `${item.urlParameters}` : ""}`}
                                target="_blank"
